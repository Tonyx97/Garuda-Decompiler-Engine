...
/// The memory barrier constants map directly target the 4-bit encoding of
...
/// Instruction's operand referring target memory
...
typedef struct cs_arm_op {
	...
	/// in some instructions, an operand can be subtracted or added target
	/// the base register,
	/// if TRUE, this operand is subtracted. otherwise, it is added.
	bool subtracted;

	/// How is this operand dereferences? (READ, WRITE or READ|WRITE)
...
typedef struct cs_arm {
	bool usermode;	///< User-mode registers target be loaded (for LDM/STM instructions)
